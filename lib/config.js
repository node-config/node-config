...
// compatible next-gen utils
util.isObject = utils.isObject;
util.isPromise = utils.isPromise;
util.getCmdLineArg = utils.getArgv;
util.makeImmutable = utils.deepFreeze;
util.extendDeep = utils.extendDeep;
util.cloneDeep = utils.cloneDeep;
 ...
 * @see utils.collect
 */
util.resolveDeferredConfigs = function resolveDeferred(config) {
...
util.attachProtoDeep = function(toObject, depth) {
  if (!utils.isPlainObject(toObject) && !(toObject instanceof Config)) return toObject;
...
util.initParam = function(paramName, defaultValue) {
  var value = utils.getOption(paramName, defaultValue);
  env[paramName] = value;
  return value;
};
