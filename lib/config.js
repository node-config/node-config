...
util.loadFileConfigs = function() {
    ...
    // Override configurations from custom environment variables
    var customEnvVars = util.getCustomEnvVars(CONFIG_DIR, extNames);
    // TODO: discuss order here
    console.log(customEnvVars);
    util.extendDeep(config, customEnvVars);

    // Place the mixed NODE_CONFIG and custom environment variables into the environment
...
util.parseFile = function(fullFilename) {
    ...
    try {
        if (extension === 'yaml' || extension === 'yml') {
            ...
            if (Yaml) {
                configObject = Yaml.load(util.stripYamlComments(fileContent));
...
util.substituteDeep = function(substitutionMap, variables){
    ...
    function _substituteVars(map, vars, pathTo){
        // console.log('Entering with: ', pathTo);
        for (var prop in map) {
            var value = map[prop];
            if (typeof(value) === 'string') {
                // We found a leaf variable name
                // console.log('setting',pathTo, value);
                if (vars[value]) {
                    // if the vars provide a value
...
/**
 * Return a deep inversion of the specified object.
 *
 * This returns a new object with all elements inverted in a shallow object,
 * where final values have become keys and the path to those values has
 * become a list of the keys to the value. An implication of this is duplicate
 * keys may overwrite
 *
 * @protected
 * @method invertDeep
 * @param someObject {object} The original object to invert
 * @return {object} the inverted object
 */
util.invertDeep = function(someObject) {
    var result = {};
    _invert(someObject, []);
    return result;

    function _invert(partialObjet, pathTo) {
        for (var prop in partialObjet) {
            pathTo.push(prop);
            var value = partialObjet[prop];
            if (typeof(value) === 'string') {
                result[value] = pathTo.slice(0); // clone of array
            } else if (util.isObject(value)) {
                // console.error("Object value", value);
                _invert(value, pathTo.slice(0));
            } else {
                msg = "Illegal key type for inversion at " + pathTo.join('.') + ': ' + typeof(value);
                throw Error(msg);
            }
            pathTo.pop();
        }
