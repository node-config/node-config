...
/**
 * Options that influence config loading.
 *
 ...
 * @property {boolean=} skipConfigSources
 * @property {boolean=} gitCrypt
 ...
 * @module config
 * @constructor
 * @return {Config} CONFIG - The top level configuration object
 */
 ...
 */
const util = ConfigClass.prototype.util = {};
 ...
 */
ConfigClass.prototype.get = function(property) {
 ...
 */
ConfigClass.prototype.has = function(property) {
...
util.attachProtoDeep = function(toObject, depth) {
  ...
  // Adding ConfigClass.prototype methods directly to toObject as hidden properties
  // because adding to toObject.__proto__ exposes the function in toObject
  for (const fnName in ConfigClass.prototype) {
    if (!toObject[fnName]) {
      Util.makeHidden(toObject, fnName, ConfigClass.prototype[fnName]);
    }
  }
 ...
 *
 * @param {any} config The part of the config to copy and serialize. Omit this argument to return the entire config.
...
/** @type {Config} */
const config = module.exports = new ConfigClass();
