...
var DEFAULT_CLONE_DEPTH = 20,
    ...
    cmdLineArgs = process.argv.slice(2, process.argv.length);
...
Config.prototype._loadFileConfigs = function() {
  ...
  // Get the deployment type from NODE_ENV
    var deployment = process.env.NODE_ENV || t.getCmdLineArg('NODE_ENV') || 'development';
 ...
 */
Config.prototype.getCmdLineArg = function (searchFor) {
    ...
    for (var argvIt in cmdLineArgs) {
        ...
        if (previouslyMatched) {
            return cmdLineArgs.splice(argvIt - 1, 2)[1];
        }

        if (match) {
            if (match[1]) {
                cmdLineArgs.splice(argvIt, 1);
                return match[1];
            }
            else {
