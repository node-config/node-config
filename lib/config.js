...
class Config {
  ...
  constructor(load) {
    ...
    this.util = new ConfigUtils(this);
  ...
  get(property) {
    ...
    if (checkMutability) {
      if (!this[LOAD_SYMBOL].initParam('ALLOW_CONFIG_MUTATIONS', false)) {
        this.util.makeImmutable(this);
      }
...
class ConfigUtils {
  ...
  withModuleDefaults(moduleName, defaultProperties) {
    const load = this.#config[LOAD_SYMBOL];
    const copy = new Config(load.clone());
