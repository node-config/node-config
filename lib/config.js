...
var Config = function() {
  ...
  // Bind all util methods to this
  for (var fnName in util) {
    util[fnName] = util[fnName].bind(t);
  }

  // Merge configurations into this
  util._extendDeep(t, util._loadFileConfigs());
  util._attachProtoDeep(t);
...
Config.prototype.has = function(property) {
  var t = this;
  return (util._get(t, property) !== undefined);
};
 ...
 */
util._get = function(object, property) {
  var t = this,
  ...
  return util._get(value, elems.slice(1));
};
    ...
    if (typeof(o.__propertyValues[prop]) == 'undefined') {
      ...
      Object.defineProperty(o, prop, {
        ...
        set : function(newValue) {
          ...
          if (util._equalsDeep(origValue, newValue))
            return;
...
util.setModuleDefaults = function(moduleName, defaultProperties) {
  ...
  var t = this,
      moduleConfig = util._cloneDeep(defaultProperties);
  ...
  util._extendDeep(configSources[0].parsed[moduleName], defaultProperties);

  // Attach handlers & watchers onto the module config object
  util._attachProtoDeep(moduleConfig);
  ...
  util._extendDeep(t[moduleName], moduleConfig);
 ...
 */
util._loadFileConfigs = function() {
  ...
  NODE_ENV = util._initParam('NODE_ENV', 'development');
  CONFIG_DIR = util._initParam('NODE_CONFIG_DIR', process.cwd() + '/config');
  if (CONFIG_DIR.indexOf('.') === 0) {
    CONFIG_DIR = process.cwd() + '/' + CONFIG_DIR;
  }
  APP_INSTANCE = util._initParam('NODE_APP_INSTANCE');
  HOST = util._initParam('HOST');
  HOSTNAME = util._initParam('HOSTNAME');

  // This is for backward compatibility
  RUNTIME_JSON_FILENAME = util._initParam('NODE_CONFIG_RUNTIME_JSON', CONFIG_DIR + '/runtime.json');
  ...
  baseNames.forEach(function(baseName) {
    extNames.forEach(function(extName) {
      ...
      var configObj = util._parseFile(fullFilename);
      if (configObj) {
        util._extendDeep(config, configObj);
      }

      // See if the application instance file is available
      if (APP_INSTANCE) {
        fullFilename = CONFIG_DIR + '/' + baseName + '-' + APP_INSTANCE + '.' + extName;
        configObj = util._parseFile(fullFilename);
        if (configObj) {
          util._extendDeep(config, configObj);
        }
      }
  ...
  if (process.env.NODE_CONFIG) {
    ...
    util._extendDeep(config, envConfig);
    configSources.push({
      name: "$NODE_CONFIG",
      parsed: envConfig,
    });
  ...
  var cmdLineConfig = util._getCmdLineArg('NODE_CONFIG');
  if (cmdLineConfig) {
    try {
    ...
    util._extendDeep(config, cmdLineConfig);
    configSources.push({
  ...
  runtimeJson = util._parseFile(RUNTIME_JSON_FILENAME) || {};
  util._extendDeep(config, runtimeJson);
 ...
 */
util._parseFile = function(fullFilename) {
  ...
  try {
    if (extension === 'yaml' || extension === 'yml') {
      ...
      if (Yaml) {
        configObject = Yaml.load(util._stripYamlComments(fileContent));
      }
      else if (VisionmediaYaml) {
        // The yaml library doesn't like strings that have newlines but don't
        // end in a newline: https://github.com/visionmedia/js-yaml/issues/issue/13
        fileContent += '\n';
        configObject = VisionmediaYaml.eval(util._stripYamlComments(fileContent));
      }
      else {
      ...
      configObject = JSON.parse(util._stripComments(fileContent));
    }
 ...
 * @return toObject
 */
util._attachProtoDeep = function(toObject, depth) {
  ...
  for (var prop in toObject) {
    ...
    // Call recursively if an object
    if (util._isObject(toObject[prop])) {
      util._attachProtoDeep(toObject[prop], depth - 1);
    }
  }
 ...
 */
util._cloneDeep = function(obj, depth) {
  ...
  for (var prop in obj) {
    ...
    if (obj[prop] && typeof obj[prop] == 'object') {
      copy[prop] = util._cloneDeep(obj[prop], depth - 1);
    }
    else {
      copy[prop] = obj[prop];
    }
 ...
 */
util._equalsDeep = function(object1, object2, depth) {
  ...
  for (var prop in object1) {
    ...
    if (object1[prop] && typeof(object1[prop]) === 'object') {
      if (!util._equalsDeep(object1[prop], object2[prop], depth - 1)) {
        return false;
      }
    }
    else {
 ...
 *                  result in object2.
 */
util._diffDeep = function(object1, object2, depth) {
  ...
  for (var parm in object2) {
    ...
    if (value1 && value2 && util._isObject(value2)) {
      if (!(util._equalsDeep(value1, value2))) {
        diff[parm] = util._diffDeep(value1, value2, depth - 1);
      }
    }
 ...
 */
util._extendDeep = function(mergeInto) {
  ...
  vargs.forEach(function(mergeFrom) {
    ...
    for (var prop in mergeFrom) {
      ...
      if (util._isObject(mergeInto[prop]) && util._isObject(mergeFrom[prop])) {
        util._extendDeep(mergeInto[prop], mergeFrom[prop], depth - 1);
      }

      // Copy recursively if the mergeFrom element is an object (or array or fn)
      else if (mergeFrom[prop] && typeof mergeFrom[prop] == 'object') {
        mergeInto[prop] = util._cloneDeep(mergeFrom[prop], depth - 1);
      }
 ...
 */
util._stripYamlComments = function(fileStr) {
 ...
 * @return {string} The string with comments stripped.
 */
util._stripComments = function(fileStr) {
 ...
 */
util._initParam = function (paramName, defaultValue) {
  var t = this;
  return util._getCmdLineArg(paramName) || process.env[paramName] || defaultValue;
}
 ...
 */
util._getCmdLineArg = function (searchFor) {
...
// Produce warnings on top level utility usage
var utilWarnings = {};
['_get', 'watch', 'setModuleDefaults', 'makeHidden', 'makeImmutable', 'getConfigSources', '_loadFileConfigs',
 '_parseFile', '_attachProtoDeep', '_cloneDeep', '_equalsDeep', '_diffDeep', '_extendDeep', '_stripYamlComments',
 '_stripComments', '_isObject', '_initParam', '_getCmdLineArg'].forEach(function(fnName) {
  Config.prototype[fnName] = function(){

    // Produce the warning
    if (!utilWarnings[fnName]) {
      console.error('WARNING: config.' + fnName + '() is deprecated.  Use config.util.' + fnName + '() instead.');
      console.error('WARNING: See https://github.com/lorenwest/node-config/wiki/Future-Compatibility#upcoming-incompatibilities');
      utilWarnings[fnName] = true;
    }

    // Forward the call
    return util[fnName].apply(this, arguments);
  }
});